...
class MixLoss(nn.Module):
    ...
    def forward(self, processed_img, reference_img):
        ...
        if reference_img.shape[2] < 176:
            #print("padding")
            padding = (24, 24, 24, 24)
            processed_img = F.pad(processed_img, padding, mode='reflect')
            reference_img = F.pad(reference_img, padding, mode='reflect')
        #print(reference_img.shape, processed_img.shape)
